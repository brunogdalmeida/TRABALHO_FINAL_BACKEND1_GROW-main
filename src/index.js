...
app.post('/signup', async (req, res) =>{
    ...
    if(!password){
        res.status(400).send(JSON.stringify({ Mensagem: "Senha da pessoa usuária inválida.Favor inserir dado" }))
    }
...
app.post('/login', async (req,res)=>{
    ...
    if(!findEmail){
        res.status(400).send(JSON.stringify({ Mensagem: "Email não encontrado em nossa base de dados." }))
    }
    ...
    if(findEmail){
        ...
        if(passwordMatched){
            res.status(200).send(JSON.stringify({ Mensagem: `Seja bem vinde ${findEmail.name} ! Pessoa usuária logada com sucesso!` }))
        }else{
            send(JSON.stringify({ Mensagem: `Credenciais não válidas. Verifique os dados` }))
        }
    }
})
...
app.post('/message/:email',(req,res)=>{
    ...
    if(!email){
        res.status(400).send(JSON.stringify({ Mensagem: "Por favor, verifique se passou um email válido" }))
    }
    ...
    if(!title){
        res.status(400).send(JSON.stringify({ Mensagem: "Por favor, verifique se passou um titulo válido" }))
    }
    //Verifica se passou a descricao da mensagem
    if(!description){
        res.status(400).send(JSON.stringify({ Mensagem: "Por favor, verifique se passou uma descrição válida" }))
    }
...
app.get('/message/:email',(req,res)=>{
    ...
    if(messages.length ===0){
        res.status(400).send(JSON.stringify({ Mensagem: "Não há mensagem regidtradas. Por gentileza, registre uma mensagem." }))
    }
    ...
    //Retorna a resposta
    res.status(200).send(JSON.stringify({ Mensagem: `Seja bem-vinde! As mensagens cadastradas são : ${listOfMessage}` }))
})
...
app.put('/message/:id',(req,res)=>{
    ...
    if(findMessageIdIndex === -1) {
        res.status(400).send(JSON.stringify({ Mensagem: `Mensagem cujo o id é : ${id}, não foi localizado no nosso banco de dados`}))
    }
...
app.delete('/message/:id',(req,res)=>{
    ...
    if(findMessageIdIndex !== -1){
        ...
        res.status(200).send(JSON.stringify({ Mensagem: `Mensagem apagada com sucesso`}))
    }
